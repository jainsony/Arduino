...
a_speed = map(a, 0, 1023, 0, move);
b_speed = map(b, 0, 1023, 0, move);
c_speed = map(c, 0, 1023, 0, move);
d_speed = map(d, 0, 1023, 0, move);
...
void control()
{
  ...
  else if(b > threshold)
  {
    ...
    analogWrite(PWM2, b_speed); //right motor bck
  ...
  else if(c > threshold)
  {
    // c = analogRead(IN3);
    // a_speed = c;
    analogWrite(PWM1, c_speed); //left motor fwd
    digitalWrite(DIR1, LOW);
  ...
  else if(d > threshold)
  {
    // d = analogRead(IN4);
    // a_speed = d;
    analogWrite(PWM1, d_speed); //left motor bck
